...
public:
	...
	// Check if start and end tiles are in a straight line
	UFUNCTION(BlueprintPure, Category = "Tilemap")
	bool IsStraightLine(FIntPoint StartTile, FIntPoint EndTile);

	// Check if there are any blocked tiles between the start and end tiles
	...
	// Remove the fishnet from the given tile.
